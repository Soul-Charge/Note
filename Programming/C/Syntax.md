...
**标识符命名规则：** 大小写字母、数字、下划线，美元符号`$`，区分大小写并且不能以数字开头  
...
|           | unsigned int | int     | 八进制格式int | 带符号八进制格式int | 十六进制格式int | 带符号十六进制格式int |
| :-------- | :----------- | :------ | :----------- | :----------------- | -------------- | ------------------- |
|           | `%u`         | `%d`    | `%o`         | `%#o`              | `%x`           | `%#x`               |
| short     | `%hu`        | `%hd`   | `%ho`        | `%#ho`             | `%hx`          | `%#hx`              |
| long      | `%lu`        | `%ld`   | `%lo`        | `%#lo`             | `%lx`          | `%#lx`              |
| long long | `%llu`       | `%lld`  | `%llo`       | `%#llo`            | `%llx`         | `%#llx`             |
| long long | `%I64u`      | `%I64d` | `%I64o`      | `%#I64o`           | `%I64x`        | `%#I64x`            |

**关于编译器警告**  
如果编译器已经使用c99标准，但使用`%lld`或`%llu`出现警告(如下)，则将`ll`改为`I64`  

> warning: unknown conversion type character 'l' in format [-Wformat=]  

[关于此问题详见此](https://stackoverflow.com/questions/13590735/printf-long-long-int-in-c-with-gcc/13590809#13590809?newreg=09aca869403c48418620efef359f5b9b)  

**其他**  
如果系统的int和long类型长度相同则可以用%d输出long类型，但可能对把程序移植到其他系统带来麻烦用%d输出short类型同理  
使用%d和%h输出short类型变量的结果相同，因为传递参数时，short类型会自动转为int类型  
...
| :---------- | :------------------ | ---------------------- | ----------------------- |
| 一般大小     | 16位                | 32位                   | 64位                    |
| 最小取值范围 | 0~65535(0~2^16-1)   | 0~4294967295(0~2^32-1) | 0~2^64-1                |
...
有符号整数超过最大值会从最小值开始，无符号整数的情况C标准未定义，可能与有符号整数相同情况  
溢出不只是指变量的值，使用printf()输出时一样(数值相对于格式说明符的溢出，相关见常量表示）  
...
**编译器自动选择存储类型**  
C源码中的整数一般以int类型存储，当int类型不足以存储时按照long, unsigned long, long long, unsigned long long存储  
八进制和十六进制数通常为int类型，数值过大则依次使用unsigned int, long, unsigned long, long long, unsigned long long 
整数：不含小数点的数  

**手动指定存储类型**  
在数字后加`l`或`L`， `ll`或`LL`， `u`或`U`表示将数字按照指定类型存储，对八进制或十六进制同样适用  
`u`或`U`可与其他两种自由组合  

|     | long      |  long long  |                             unsigned                              |
| :-- | :-------- | ----------- | ----------------------------------------------------------------- |
| 后缀 | `l`或`L`  | `ll`或`LL`  | `u`或`U`                                                          |
| 例   | `5l` `5L` | `5ll` `5LL` | `5u` `5U` `5llu` `5ull` `5llU` `5Ull` `5LLu` `5uLL` `5LLU` `5ULL` |
